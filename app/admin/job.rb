ActiveAdmin.register Delayed::Job, as: 'Job' do
  ...
  action_item :only => [:edit] do
    link_to 'Delete Job', grubraise_job_path(resource),
            'data-method' => :delete, 'data-confirm' => 'Are you sure?'
  end
 
  action_item :only => [:show, :edit] do
    link_to 'Schedule now', run_now_grubraise_job_path(resource), 'data-method' => :post,
      :title => 'Cause a job scheduled in the future to run now.'
  end
 
  action_item :only => [:show, :edit] do
    link_to 'Reset Job', reset_grubraise_job_path(resource), 'data-method' => :post,
